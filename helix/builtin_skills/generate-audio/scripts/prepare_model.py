...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    if status_code != 200 or parsed.get("status") != "ok":
        return _err(
            error_code=str(parsed.get("error_code", "")).strip() or "audio_prepare_failed",
            message=str(parsed.get("message", "")).strip() or body.strip() or "audio prepare failed",
        ), 1

    return _ok(
        message=str(parsed.get("message", "")).strip() or "audio model prepared",
    ), 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare the built-in local PyTorch text-to-audio model.")
...
def main() -> int:
    ...
    try:
        ...
        out = _err(
            error_code="audio_prepare_unexpected_exception",
